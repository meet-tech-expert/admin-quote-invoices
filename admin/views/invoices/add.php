...
*/
$id = '0';
$head_text = "Add New Invoice";
$result = false;
global $wpdb;
//$invoiceTable = $wpdb->prefix. 'admin_invoices';
...
if( isset($_REQUEST['action']) && isset($_REQUEST['id']) && ($_REQUEST['action'] == 'edit' && $_REQUEST['id']!='') && filter_var($_REQUEST['id'], FILTER_VALIDATE_INT) ){
	...
	$result   =  $model->edit_invoice($_REQUEST['id']);
	if($result){
		$id = $_REQUEST['id'];
		$head_text = "Edit Invoice";
...
<div class="wrap">
	...
	<div id="poststuff" class="">
		<div id="post-body">
			...
			<a href="admin.php?page=<?php echo $_REQUEST['page'] ?>"><?php esc_html_e('All Invoices',$this->plugin_name); ?></a>
			...
			<div id="post-body-content">
				...
				<input type="hidden" name="edit_id" value="<?php echo $id;?>"/>
					...
					wp_nonce_field( 'invoice_add_action', 'invoice_add_nonce_field' );
					?>
					...
					<table class="form-table">
						...
						<tr valign="top">
							...
							<td>
								<input type="text" name="serial_code" value="<?php echo ($result)? $result['invoice']['serial_code']: ''; ?>" placeholder="<?php echo 'esempio '. date("Y").'000';?>" pattern="[0-9]{7}" title="<?php echo 'Inserisci 7 cifre solo con il formato '. date("Y").'000';?>" required=""  autofocus />
							</td>
						</tr>
						...
						<tr valign="top">
							...
							<td>
								<input type="text" name="object" value="<?php echo ($result)? $result['invoice']['object']:''; ?>" required=""/>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row">
							...
							<td>
								<select name="customer" required="">
									...
									<option value="1" <?php echo ($result && $result['invoice']['customer']=='1')? 'selected' :''; ?>>Mario</option>
									<option value="2" <?php echo ($result && $result['invoice']['customer']=='2')? 'selected' :''; ?>>David</option>
								</select>
							</td>
						</tr>
						<!--<tr valign="top">
							...
							<td>
								<input type="text" name="creation_date" class="admin-date" value="<?php echo ($result)? $result['invoice']['creation_date']:''; ?>" required=""/>
							</td>
						</tr>-->
						<tr valign="top">
							<th scope="row">
								<label class="required"><?php esc_html_e("Date of issue", $this->plugin_name); ?></label>
							</th>
							<td>
								<input type="text" name="issue_date" class="admin-date" value="<?php echo ($result)? $result['invoice']['issue_date']:''; ?>" required=""/>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row">
					...
					<table class="form-table">
						<tr valign="top">
							...
							<td>
								<input type="text" name="notes" class="" value="<?php echo ($result)? $result['invoice']['notes']:''; ?>"/>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row">
							...
							<td>
								<input type="text" name="private_notes" class="" value="<?php echo ($result)? $result['invoice']['private_notes']:''; ?>"/>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row">
								<label class=""><?php esc_html_e("Paid", $this->plugin_name); ?></label>
							</th>
							<td>
								<input type="checkbox" name="paid" class="" <?php echo ($result && $result['invoice']['paid']=='yes')? 'checked' :'';  ?>  value="1"/>
							</td>
						</tr>
						<tr valign="top">
							<th scope="row">
								<label class=""><?php esc_html_e("Payment due date", $this->plugin_name); ?></label>
							</th>
							<td>
								<input type="text" name="pay_due_date" class="admin-date" value="<?php echo ($result)? $result['invoice']['pay_due_date']:''; ?>" />
							</td>
						</tr>
						<tr valign="top">
							<th scope="row">
								<label class=""><?php esc_html_e("Effective Payment date", $this->plugin_name); ?></label>
							</th>
							<td>
								<input type="text" name="effective_pay_date" class="admin-date" value="<?php echo ($result)? $result['invoice']['effective_pay_date']:''; ?>" />
							</td>
						</tr>
					...
					<div style="clear: both;">
						<p class="submit">
							...
							<?php $sts = ($result)? $result['invoice']['issue_invoice']:'no'; 
							if($sts == 'no'){
							?>
							<input type="submit" name="issue_invoice" class="button button-primary button-large" value="<?php _e('Save and Issue Invoice') ?>" />
							<?php } ?>
						</p>
					</div>		
				</form>
