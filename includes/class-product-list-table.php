...
class Product_List_Table extends WP_List_Table {
  ...
  public static function get_records( $per_page, $page_number = 1 ) {
		...
		global $wpdb;
		$search_product   = ( isset( $_REQUEST['filter_name'] ) ) ? $_REQUEST['filter_name'] : false;
		
		$sql = "SELECT *  FROM {$wpdb->prefix}admin_products";
		$where = ' WHERE 1';
		
		if($search_product){
			$where .= " AND name LIKE '%".$search_product."%' ";
		}
	...
	public static function record_count() {
		...
		$sql = "SELECT COUNT(*) FROM {$wpdb->prefix}admin_products";
  ...
  function column_default( $item, $column_name ) {
	...
	function get_columns(){
	        $columns = array(
	            ...
	            'name' => __( 'Product name', $this->plugin_name),
	            'code' => __( 'Code', $this->plugin_name ),
	...
	function column_name($item){
		$delete_nonce = wp_create_nonce( 'product_delete_record' );
	  $actions = array(
	            'edit'      => sprintf('<a href="?page=%s&action=%s&id=%s">modificare</a>','products','edit',$item['id']),
	            'delete' => sprintf( '<a class="delete_record" href="?page=%s&action=%s&productId=%s&_wpnonce=%s">Elimina</a>', esc_attr( $_REQUEST['page'] ), 'delete', absint( $item['id'] ), $delete_nonce ),
	        );
	...
	public function process_bulk_action() {
		...
		if ( 'delete' === $this->current_action() ) {
			...
			if ( ! wp_verify_nonce( $nonce, 'product_delete_record' ) ) {
				die( 'Go get a life script kiddies' );
			}
			else {
				self::delete_record( absint( $_GET['productId'] ) );
	    ...
	    if ( $which == "top" ){
	        ...
	        <div class="alignleft actions bulkactions">
	         <input type="hidden" name="page" value="<?php echo $_REQUEST['page'] ?>" />
	       		<input type="search" id="quote-search-input" name="filter_name" value="<?php echo ( isset( $_REQUEST['filter_name'] ) ) ? $_REQUEST['filter_name'] : '';?>" placeholder="<?php esc_html_e('Filter Product',$this->plugin_name)?>">
